...
class ElastAlerter():
    ...
    def get_metric_agg_query(self, query, agg, field):
        """ Takes a query generated by get_query and outputs a aggregation query """
        if 'sort' in query:
            query.pop('sort')
        query.update({'aggs': {'agg': {agg: {'field': field}}}})
        aggs_query = {'aggs': {'filtered': query}}
        return aggs_query
    ...
    def get_hits_agg(self, rule, starttime, endtime, index, agg, key):
        """ Query elasticsearch for the aggregation of result values and returns a list of timestamps
        equal to the endtime. This allows the results to be passed to rules which expect
        an object for each hit.
        ...
        agg_val = res['aggregations']['filtered']['agg']['value']
        lt = rule.get('use_local_time')
        elastalert_logger.info('Queried rule %s from %s to %s: %s = %s' % (rule['name'], pretty_ts(starttime, lt), pretty_ts(endtime, lt), agg, agg_val))
        return {endtime: agg_val}
    ...
    def run_query(self, rule, start=None, end=None):
        ...
        elif rule.get('use_agg_query'):
            data = self.get_hits_agg(rule, start, end, index, rule['agg_type'], rule['agg_field'])
